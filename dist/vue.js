...
(function (global, factory) {
  ...
  var Observer = function Observer(value) {
    ...
    if (Array.isArray(value)) {
      // export const hasProto = '__proto__' in {} 看是否能使用隐式原型
      if (hasProto) {
        // 指定隐式原型为Array
  ...
  function defineReactive(
    ...
    // TODO Dep的作用,一个Dep对应一个key，因为成为了一个闭包，所以能一一对应
    var dep = new Dep();
  ...
  function set(target, key, val) {
    ...
    // 数组更新指定下标元素，通过splice来触发更新
    if (Array.isArray(target) && isValidArrayIndex(key)) {
      target.length = Math.max(target.length, key);
      target.splice(key, 1, val);
      return val;
    }
    // 如果是自己已经存在的属性，则重新设置，并返回
  ...
  function nextTick(cb, ctx) {
    ...
    callbacks.push(function () {
      if (cb) {
        ...
        // 使promise变成fullfiedld状态
        _resolve(ctx);
      }
    });
    if (!pending) {
      pending = true;
      // 执行flushCallbacks方法，执行所有的callback
      timerFunc();
    }
    // $flow-disable-line
    // 如果回调为空，返回一个Promise
  ...
  function resolveInject(inject, vm) {
    if (inject) {
      ...
      // TODO Object.keys与Reflect.ownKeys的区别
  ...
  // 生成节点：_c('div',{attrs:{"id":"app"}},[_c('span',[_v(_s(msg))]),_v(" "),_c('span',[_v(_s(abc))]),_v(" "),_c('button',{on:{"click":doChange}},[_v("点击修改")])])
  ...
  function mountComponent(
    ...
    if ( config.performance && mark) {
      ...
      updateComponent = function () {
        // 执行render函数，获取到VNode，再调用_update
  ...
  function initMixin(Vue) {
    Vue.prototype._init = function (options) {
      ...
      /* istanbul ignore else */
      {
        // TODO 设置代理，initProxy作用是什么？
        initProxy(vm);
      }
      // expose real self
      vm._self = vm;
      // 初始化组件实例关系属性：$parent,$children,等
      initLifecycle(vm);
      // 初始化自定义事件
      initEvents(vm);
      // 解析插槽信息
  ...
  function parseHTML(html, options) {
    ...
    function parseStartTag() {
      ...
      if (start) {
        ...
        var end, attr;
        // end表示开始标签对应结束标签的位置；attr表示属性的位置
        while (
          !(end = html.match(startTagClose)) &&
          (attr = html.match(dynamicArgAttribute) || html.match(attribute))
        ) {
          // 进到这里表示当前标签没有结束,获取他们的属性
          attr.start = index;
  ...
  function parse(
    ...
    parseHTML(template, {
      ...
      start: function start(tag, attrs, unary, start$1, end) {
        ...
        // 如果root为空，当前elment为root
        if (!root) {
          root = element;
          {
            // 提示不能使用solt和template为根
